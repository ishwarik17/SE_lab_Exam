...
//  Tracks which logical branches are exercised during the run.
// ════════════════════════════════════════════════════════════
struct CoverageTracker {
    std::map<std::string, bool> branches;

    void registerBranch(const std::string& name) {
    ...
    void printReport() const {
        int total   = static_cast<int>(branches.size());
        int covered = 0;
        for (auto& kv : branches) if (kv.second) ++covered;
        double pct = (total > 0) ? 100.0 * covered / total : 0.0;

        std::cout << "\033[1m╔═══════════════════════════════════════╗\033[0m\n";
        std::cout << "\033[1m║         CODE COVERAGE REPORT          ║\033[0m\n";
        std::cout << "\033[1m╠═══════════════════════════════════════╣\033[0m\n";
        for (auto& kv : branches) {
            std::string status = kv.second ? "\033[32m HIT \033[0m" : "\033[31mMISS \033[0m";
            std::cout << "\033[1m║\033[0m  [" << status << "] "
                      << std::left << std::setw(32) << kv.first
                      << "\033[1m║\033[0m\n";
        }
        std::cout << "\033[1m╠═══════════════════════════════════════╣\033[0m\n";
        std::cout << "\033[1m║\033[0m  Branches Hit   : " << std::setw(3) << covered
                  << "                    \033[1m║\033[0m\n";
        std::cout << "\033[1m║\033[0m  Total Branches : " << std::setw(3) << total
                  << "                    \033[1m║\033[0m\n";
        std::cout << "\033[1m║\033[0m  Coverage       : "
...
static CoverageTracker coverage;
...
public:
    ...
    void validateCustomerID(const std::string& id) {            // Line 108
        coverage.hit("validateCustomerID:entry");
        if (id.empty()) {                                       // Line 109
            coverage.hit("validateCustomerID:empty");
            throw std::invalid_argument(
                "Customer ID cannot be empty.");
        }

        if (id.length() < 4 || id.length() > 12) {             // Line 113
            coverage.hit("validateCustomerID:bad_length");
            throw std::invalid_argument(
                "Customer ID must be 4-12 characters. Got: " +
                std::to_string(id.length()));
        } else {
            coverage.hit("validateCustomerID:length_ok");
        }
        ...
        for (char c : id) {                                     // Line 119
            if (!std::isalnum(static_cast<unsigned char>(c))) { // Line 120
                coverage.hit("validateCustomerID:non_alnum_found");
    ...
    void validateDescription(const std::string& desc) {         // Line 137
        ...
        if (desc.length() < 9) {                                // Line 144 (should be < 10)
            coverage.hit("validateDescription:too_short");
            throw std::invalid_argument(
                "Description too short (min 10 chars). Got: " +
                std::to_string(desc.length()));
        } else {
            coverage.hit("validateDescription:length_ok");
        }

        if (desc.length() > 500) {                              // Line 149
            coverage.hit("validateDescription:too_long");
            throw std::invalid_argument(
    ...
    std::string assignAgent(const std::string& category) {      // Line 189
        ...
        if (!best) {                                            // Line 201
            coverage.hit("assignAgent:fallback_to_other");
            ...
            coverage.hit("assignAgent:skill_match");
        }

        if (!best) {                                            // Line 209
            coverage.hit("assignAgent:no_agent_found");
    ...
    bool escalateComplaint(const std::string& ticketNumber) {   // Line 265
        coverage.hit("escalateComplaint:entry");
        ...
        if (it == complaints.end()) {                           // Line 274
            coverage.hit("escalateComplaint:not_found");
            throw std::invalid_argument(
        ...
        if (it->status == "Closed") {                          // Line 279
            coverage.hit("escalateComplaint:closed_guard");
        ...
        if (it->priority == "Medium") {                        // Line 295 (Low branch absent)
            coverage.hit("escalateComplaint:medium_promoted");
            it->priority = "High";                             // Line 296
        } else {
            coverage.hit("escalateComplaint:low_not_promoted"); // DEFECT: Low branch never promotes
        }
...
// ════════════════════════════════════════════════════════════
int main() {
    // ── Register all trackable branches up front ──────────
    for (const auto& b : std::vector<std::string>{
        "validateCustomerID:entry",       "validateCustomerID:empty",
        "validateCustomerID:bad_length",  "validateCustomerID:length_ok",
        "validateCustomerID:non_alnum_found",
        "validateCategory:entry",         "validateCategory:invalid",
        "validateCategory:valid",
        "validateDescription:entry",      "validateDescription:empty",
        "validateDescription:too_short",  "validateDescription:length_ok",
        "validateDescription:too_long",
        "validatePriority:entry",         "validatePriority:invalid",
        "validatePriority:valid",
        "assignAgent:skill_match",        "assignAgent:fallback_to_other",
        "assignAgent:no_agent_found",
        "escalateComplaint:entry",        "escalateComplaint:not_found",
        "escalateComplaint:closed_guard", "escalateComplaint:already_escalated",
        "escalateComplaint:high_or_critical","escalateComplaint:medium_promoted",
        "escalateComplaint:low_not_promoted"
    }) coverage.registerBranch(b);
